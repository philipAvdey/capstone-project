...
class HandTrajectoryNode(Node):
    def __init__(self, name):
        ...
        self.point_publisher = self.create_publisher(Points, '~/points', 1)  # 使用~可以自动加上前缀名称
    ...
    def enter_srv_callback(self, request, response):
        ...
        if self.image_sub is None:
             self.image_sub = self.create_subscription(Image, '/ascamera/camera_publisher/rgb0/image', self.image_callback, 1)  # 摄像头订阅
        response.success = True
